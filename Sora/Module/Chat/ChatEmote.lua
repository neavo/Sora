...
local Path = "Interface\\AddOns\\Sora\\Media\\Emotes\\"
...
local function ReplaceEmote(value)
    ...
    for _, v in ipairs(emotes) do
        if (emote == v.key or emote == v.zhCN or emote == v.zhTW) then
            return "|T".. (v.texture or Path.. v.key) ..":16|t"
        end
    end
    return value
end
...
do
    ...
    for _, v in ipairs(emotes) do
        ...
        if (v.texture) then
            ...
            button:SetNormalTexture(Path .. v.key)
        end
...
if (GetCVarBool("chatBubbles")) then
    ...
    frame:SetScript("OnUpdate", function(self, elapsed)
        ...
        if (not self.paused and self.timer > 0.15) then
            self.timer = 0
