...
function SD.CreateInstance(parent, fontSize)
    ...
    frame.minValue = S.MakeText(frame, fontSize)
    frame.minValue:SetPoint("BOTTOMLEFT", frame, "BOTTOMLEFT", 0, 0)

    frame.curValue = S.MakeText(frame, fontSize)
    frame.curValue:SetPoint("BOTTOM", frame, "BOTTOM", 0, 0)

    frame.maxValue = S.MakeText(frame, fontSize)
    frame.maxValue:SetPoint("BOTTOMRIGHT", frame, "BOTTOMRIGHT", 0, 0)
...
function SD.UpdateDataBinding(self)
    ...
    self:SetSize(
        ...
        self.text:GetHeight() + 8 + self.line:GetHeight() + 8 + self.minValue:GetHeight()
    )
end
